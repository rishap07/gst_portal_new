...
class json extends validation {
	...
	public function getGSTR2APurchaseInvoiceData($userid, $returnMonth, $array_type = true) {
		...
		$queryPurchase ='Select 
							...
							and DATE_FORMAT(pi.invoice_date,"%Y-%m") = "'.$returnMonth.'" 
							and pi.invoice_type NOT IN("revisedtaxinvoice", "receiptvoucherinvoice", "importinvoice") 
							and pi.is_deleted = "0" 
	...
	public function getGSTR2APurchaseImportInvoiceData($userid, $returnMonth, $array_type = true) {

		$queryImportPurchase ='Select 
							pi.purchase_invoice_id, 
							pi.invoice_type, 
							pi.serial_number, 
							pi.reference_number, 
							...
							INNER JOIN '.$this->getTableName('client_purchase_invoice_item').' as pii 
							ON pi.purchase_invoice_id = pii.purchase_invoice_id 
							INNER JOIN '.$this->getTableName('item').' as mi ON mi.hsn_code = pii.item_hsncode 
							where 1=1 
							and pi.added_by = '.$userid.' 
							and pii.added_by = '.$userid.' 
							and DATE_FORMAT(pi.invoice_date,"%Y-%m") = "'.$returnMonth.'" 
							and pi.invoice_type = "importinvoice" 
							and pi.is_deleted = "0" 
							and pii.is_deleted = "0" 
							and pi.is_canceled = "0" 
							GROUP By pi.serial_number, mi.item_type, pii.consolidate_rate';

		return $this->get_results($queryImportPurchase, $array_type);
	}
	...
	public function addGstr2B2B2URData($userid,$financialMonth,$arr_type=true)
	{
		...
		for($i=0; $i<count($dataArry['b2bur']); $i++)
		{
			...
			//ur_type
			if($dataArry['b2bur'][$i]['invoice_total'] >'250000' &&
			 $dataArry['b2bur'][$i]['supplier_billing_gstin_number'])
			{$ur_type='B2CL';}
	...
	public function addGstr2CDNData($userid,$financialMonth,$arr_type=true)
	{
		...
		for($i=0; $i<count($dataArry['cdn']); $i++)
		{
			...
			//ur_type
			if($dataArry['cdn'][$i]['invoice_total'] >'250000' &&
			 $dataArry['cdn'][$i]['supplier_billing_gstin_number'])
			{$ur_type='B2CL';}
	...
	public function addGstr2IMPSData($userid,$financialMonth,$arr_type=true)
	{
		...
		for($i=0; $i<count($dataArry['imp_s']); $i++)
		{
			...
			//ur_type
			if($dataArry['imp_s'][$i]['invoice_total'] >'250000' &&
			 $dataArry['imp_s'][$i]['supplier_billing_gstin_number'])
			{$ur_type='B2CL';}
	...
	public function addGstr2IMPGData($userid,$financialMonth,$arr_type=true)
	{
		...
		for($i=0; $i<count($dataArry['imp_g']); $i++)
		{
			...
			//ur_type
			if($dataArry['imp_g'][$i]['invoice_total'] >'250000' &&
			 $dataArry['imp_g'][$i]['supplier_billing_gstin_number'])
			{$ur_type='B2CL';}
