        ...
        if(!empty($dataRes))
        {
            ...
            if($this->updateMultiple($this->getTableName('client_invoice'), $dataUpdate))
            {
              ...
              //  $dataReturn = $this->get_results("select * from ".TAB_PREFIX."return where client_id='".$_SESSION['client_detail']['user_id']."'");
			    $dataReturn = $this->get_results("select * from ".TAB_PREFIX."return where return_month='".$fmonth."' and client_id='".$_SESSION['user_detail']['user_id']."' and type='gstr1'");
                //print_r($dataReturn);
