...
final class purchase extends validation {
	...
	public function generatePurchaseInvoiceHtml($invoiceid) {
		...
		$invoiceData = $this->get_results("select 
												...
												cii.total 
												from 
												" . $this->tableNames['client_purchase_invoice'] ." as ci INNER JOIN " . $this->tableNames['client_purchase_invoice_item'] ." as cii ON ci.purchase_invoice_id = cii.purchase_invoice_id where ci.purchase_invoice_id = ".$invoiceid." AND ci.added_by = '".$this->sanitize($_SESSION['user_detail']['user_id'])."' AND cii.added_by = '".$this->sanitize($_SESSION['user_detail']['user_id'])."' AND ci.is_deleted='0' AND cii.is_deleted='0'");
						...
						$mpdfHtml .= '<table style="width:100%;line-height:inherit;text-align:left;">';
							$mpdfHtml .= '<tr>';
								...
								$mpdfHtml .= '<td style="padding:5px;vertical-align:top;text-align:right;padding-bottom:20px;">';
									...
									if($invoiceData[0]->invoice_type === "Import Invoice") {

										$mpdfHtml .= '<b>Import Supply Meant:</b> ' . $invoiceData[0]->import_supply_meant . '<br>';
										if($invoiceData[0]->is_canceled == 1) { $mpdfHtml .= '<b>Canceled Invoice:</b> Canceled <br>'; }
										if($invoiceData[0]->advance_adjustment == 1) { $mpdfHtml .= '<b>Advance Adjustment:</b> Yes'; }

									} else {

										$mpdfHtml .= '<b>Supply Type:</b>' . $invoiceData[0]->supply_type . '<br>';
										if(isset($invoiceData[0]->supply_place) && $invoiceData[0]->supply_place > 0) { $mpdfHtml .= '<b>Place Of Supply:</b>' . $supply_place_data['data']->state_name .'<br>'; }
										if($invoiceData[0]->is_canceled == 1) { $mpdfHtml .= '<b>Canceled Invoice:</b> Canceled <br>'; }
						...
						$mpdfHtml .= '<table style="width:100%;line-height:inherit;text-align:left;">';
							$mpdfHtml .= '<tr>';
								$mpdfHtml .= '<td style="padding:5px;vertical-align:top;padding-bottom:40px;width:50%;">';
									...
									$mpdfHtml .= $invoiceData[0]->supplier_billing_state_name .'<br>';

									if(intval($invoiceData[0]->supplier_billing_state) == 0) {
										$billing_country_data = $this->getCountryDetailByCountryId($invoiceData[0]->supplier_billing_country);
										$mpdfHtml .= $billing_country_data['data']->country_name .'<br>';
									}
									
									if(!empty($invoiceData[0]->supplier_billing_gstin_number)) {
										$mpdfHtml .= '<b>Supplier GSTIN:</b>'. $invoiceData[0]->supplier_billing_gstin_number; 
									}
								...
								$mpdfHtml .= '<td style="padding:5px;vertical-align:top;text-align:right;padding-bottom:40px;width:50%;">';
									...
									$mpdfHtml .= html_entity_decode($invoiceData[0]->recipient_shipping_name) .'<br>';
									...
									$mpdfHtml .= $invoiceData[0]->recipient_shipping_state_name .'<br>';

									if(intval($invoiceData[0]->recipient_shipping_state) == 0) {
										$shipping_country_data = $this->getCountryDetailByCountryId($invoiceData[0]->recipient_shipping_country);
										$mpdfHtml .= $shipping_country_data['data']->country_name .'<br>';
									}

									if(!empty($invoiceData[0]->recipient_shipping_gstin_number)) {
										$mpdfHtml .= '<b>Recipient GSTIN:</b>'. $invoiceData[0]->recipient_shipping_gstin_number;
									}

									if($invoiceData[0]->invoice_type === "Import Invoice") {
										$mpdfHtml .= '<b>Import Bill Number:</b>'. $invoiceData[0]->import_bill_number .'<br>';
										$mpdfHtml .= '<b>Import Bill Date:</b>'. $invoiceData[0]->import_bill_date;
									}
