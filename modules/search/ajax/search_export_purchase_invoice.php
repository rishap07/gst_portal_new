...
//Columns to fetch from database
		...
		inv.recipient_shipping_country,
		inv.recipient_shipping_vendor_type,
		inv.recipient_shipping_gstin_number,
		it.item_name,
		it.item_hsncode,
		it.item_description,
			CASE 
			  WHEN it.is_applicable = "1" THEN "Applicable"
			  WHEN it.is_applicable = "2" THEN "Non-GST"
			  WHEN it.is_applicable = "3" THEN "Exempted"
        ...
        inner JOIN '.$master_country.'  ct on (ct.id= inv.supplier_billing_country  and ct.id= inv.recipient_shipping_country )
        inner JOIN '.$vendor_type.'  vt on (vt.vendor_id= inv.supplier_billing_vendor_type and  vt.vendor_id= inv.recipient_shipping_vendor_type)
        where 1=1';
...
$uWhere .= " AND  inv.invoice_nature='purchaseinvoice'  AND inv.is_deleted='0' AND inv.added_by='".$db_obj->sanitize($_SESSION['user_detail']['user_id'])."' ";
...
$purchaseExportArray=$db_obj->get_results($uWhere,false);
...
$dataStyle1 = new PHPExcel_Style();
			...
			->setSharedStyle($headerStyle1,  "A1:AJ1")
		...
		if($totn>0)
		{
			...
			for($i=2;$i<=$totn+1;$i++){
				...
				for($x=0; $x<count($purchaseExportArray); $x++):
					...
					$objPHPExcel->getActiveSheet()->setCellValue('I'.$i,$purchaseExportArray[$x]['supplier_billing_address']);
					$objPHPExcel->getActiveSheet()->setCellValue('J'.$i,$purchaseExportArray[$x]['supplier_billing_state_name']);
					$objPHPExcel->getActiveSheet()->setCellValue('K'.$i,$purchaseExportArray[$x]['country_name']);
					$objPHPExcel->getActiveSheet()->setCellValue('L'.$i,$purchaseExportArray[$x]['vendor_name']);
					...
					$objPHPExcel->getActiveSheet()->setCellValue('P'.$i,$purchaseExportArray[$x]['recipient_shipping_address']);
					$objPHPExcel->getActiveSheet()->setCellValue('Q'.$i,$purchaseExportArray[$x]['recipient_shipping_state_name']);
					$objPHPExcel->getActiveSheet()->setCellValue('R'.$i,$purchaseExportArray[$x]['country_name']);
					$objPHPExcel->getActiveSheet()->setCellValue('S'.$i,$purchaseExportArray[$x]['vendor_name']);
					$objPHPExcel->getActiveSheet()->setCellValue('T'.$i,$purchaseExportArray[$x]['recipient_shipping_gstin_number']);
					$objPHPExcel->getActiveSheet()->setCellValue('U'.$i,$purchaseExportArray[$x]['item_name']);
					$objPHPExcel->getActiveSheet()->setCellValue('V'.$i,$purchaseExportArray[$x]['item_hsncode']);
					$objPHPExcel->getActiveSheet()->setCellValue('W'.$i,$purchaseExportArray[$x]['item_description']);
					$objPHPExcel->getActiveSheet()->setCellValue('X'.$i,$purchaseExportArray[$x]['consolidate_rate']);
					$objPHPExcel->getActiveSheet()->setCellValue('Y'.$i,$purchaseExportArray[$x]['item_quantity']);
					$objPHPExcel->getActiveSheet()->setCellValue('Z'.$i,$purchaseExportArray[$x]['item_unit_price']);
					$objPHPExcel->getActiveSheet()->setCellValue('AA'.$i,$purchaseExportArray[$x]['consolidate_rate']);
					...
					$objPHPExcel->getActiveSheet()->setCellValue('AJ'.$i,$purchaseExportArray[$x]['description']);
					$i++;
				endfor;
			}
		}else
...
$objWriter->save(PROJECT_ROOT . UPLOAD_DIR . "/export-docs/purchase-export-invoices-".$userId.".xlsx" );
$fileName= PROJECT_URL.UPLOAD_DIR . "/export-docs/purchase-export-invoices-".$userId.".xlsx";
