...
$obj_common= new common();
	...
	$uQuery = 'select
			...
			bct.country_name as billing_country,
			bvt.vendor_name as billing_vendor_type,
			ci.billing_gstin_number,
			ci.shipping_name,
			ci.shipping_company_name as shipping_business_name,
			ci.shipping_address,
			ci.shipping_state_name,
			sct.country_name as shipping_country,
			svt.vendor_name as shipping_vendor_type,
			...
			inner join '.$state. ' st on (st.state_id = ci.supply_place)
			inner join '.$country. ' bct on (bct.id= ci.billing_country )
			inner join '.$country. ' sct on (sct.id=ci.shipping_country)
			inner join '.$vendor_type. ' bvt on (bvt.vendor_id= ci.billing_vendor_type)
			inner join '.$vendor_type. ' svt on (svt.vendor_id = ci.shipping_vendor_type)';
			
	$uWhere = $uQuery . ' where 1=1 AND ci.is_deleted="0" AND ci.added_by="'.$db_obj->sanitize($_SESSION['user_detail']['user_id']).'"';
	...
	$salesExportArray = $obj_common->replaceSpecialChar($db_obj->get_results($uWhere, false));
	$range = range('A','Z');
	...
	//for HEADING ROW
	$objPHPExcel->getActiveSheet()->freezePane('A2')->setSharedStyle($headerStyle1, "A1:AL1")->getRowDimension('1')->setRowHeight(40);
	...
	if($totn > 0){
		...
		for($i=2;$i<=$totn+1;$i++){
			...
			for($x=0; $x<count($salesExportArray); $x++):
				...
				$objPHPExcel->getActiveSheet()->setCellValue('D'.$i, $salesExportArray[$x]['ecommerce_gstin_number']);
				$objPHPExcel->getActiveSheet()->setCellValue('E'.$i, $salesExportArray[$x]['ecommerce_vendor_code']);
				...
				$objPHPExcel->getActiveSheet()->setCellValue('AC'.$i, $salesExportArray[$x]['item_unit_price']);
	...
	$objWriter->save(PROJECT_ROOT . UPLOAD_DIR . "/export-docs/sales-export-invoices-".$_SESSION['user_detail']['user_id'].".xlsx");
	$fileName= PROJECT_URL.UPLOAD_DIR . "/export-docs/sales-export-invoices-".$_SESSION['user_detail']['user_id'].".xlsx";
