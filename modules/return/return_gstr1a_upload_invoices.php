                                ...
                                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="invoice-itemtable" id="mainTable1">
                                    <thead>
                                        ...
                                        <tr>
                                            ...
                                            <td align='right'><?php if(isset($b2bItemData[0]->cgst_amount)) { echo $b2bItemData[0]->cgst_amount; } else { echo '0.00'; } ?></td>
										    <td align='right'><?php if(isset($b2bItemData[0]->sgst_amount)) { echo $b2bItemData[0]->sgst_amount; } else { echo '0.00'; } ?></td>
										    <td align='right'><?php if(isset($b2bItemData[0]->igst_amount)) { echo $b2bItemData[0]->igst_amount; } else { echo '0.00'; } ?></td>
										    <td align='right'><?php if(isset($b2bItemData[0]->cess_amount)) { echo $b2bItemData[0]->cess_amount; } else { echo '0.00'; } ?></td>
                                      ...
                                      <?php
                                            ...
                                            $b2bItemData = $obj_gstr1->get_results($b2bItemquery);
                                            $b2bquery = "select * from " . $obj_gstr1->getTableName('client_invoice_item') . " where (invoice_document_nature='creditnote' or invoice_document_nature='debitnote') and billing_gstin_number!='' and  added_by='" . $_SESSION['user_detail']['user_id'] . "' and status='1' and is_canceled='0' and is_deleted='0'  and invoice_date like '%" . $returnmonth . "%'";
                                            $b2bData = $obj_gstr1->get_results($b2bquery);
                                            $b2bTotquery = "select sum(invoice_total_value) as invoice_total_value from " . $obj_gstr1->getTableName('client_invoice_item') . " where (invoice_document_nature='creditnote' or invoice_document_nature='debitnote') and billing_gstin_number!='' and  added_by='" . $_SESSION['user_detail']['user_id'] . "' and status='1' and is_canceled='0' and is_deleted='0'  and invoice_date like '%" . $returnmonth . "%'";
                                            $b2bTotData = $obj_gstr1->get_results($b2bTotquery);
                                            $total = 0;
                                            if (!empty($b2bItemData)) {
                                                $total = $b2bItemData[0]->cgst_amount + $b2bItemData[0]->sgst_amount + $b2bItemData[0]->igst_amount + $b2bItemData[0]->cess_amount;
