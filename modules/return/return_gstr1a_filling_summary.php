...
if (isset($_POST['returnmonth'])) 
{
    ...
    $obj_gstr1->redirect(PROJECT_URL . "/?page=return_gstr1a_filling_summary&returnmonth=" . $returnmonth);
    exit();
}
        ...
        <div class="col-md-12 col-sm-12 col-xs-12 heading">
            <div class="tab col-md-12 col-sm-12 col-xs-12">
              <a href="<?php echo PROJECT_URL . '/?page=return_gstr1a_download&returnmonth=' . $returnmonth ?>"  >
                ...
                <a href="<?php echo PROJECT_URL . '/?page=return_gstr1a_filling_summary&returnmonth=' . $returnmonth ?>" class="active">
                    File GSTR-1A
                </a>        
            </div>
            <div id="upload_invoice" class="tabcontent">
                <div class="col-md-12 col-sm-12 col-xs-12">
                    <div class="col-md-6 col-sm-12 col-xs-12"><h3>GSTR-1A Filing Summary</h3></div>
...
<!--                                            <tr>
                                            ...
                                            <tr>
                                                ...
                                                $b2bItemData = $obj_gstr1->get_results($b2bItemquery);
                                                $b2bquery = "select * from " . $obj_gstr1->getTableName('client_invoice') . " where (invoice_document_nature='creditnote' or invoice_document_nature='debitnote') and billing_gstin_number!='' and  added_by='" . $_SESSION['user_detail']['user_id'] . "' and status='1' and is_canceled='0' and is_deleted='0'  and invoice_date like '%" . $returnmonth . "%' ";
                                                $b2bData = $obj_gstr1->get_results($b2bquery);
                                                $b2bTotquery = "select sum(invoice_total_value) as invoice_total_value from " . $obj_gstr1->getTableName('client_invoice') . " where (invoice_document_nature='creditnote' or invoice_document_nature='debitnote') and billing_gstin_number!='' and  added_by='" . $_SESSION['user_detail']['user_id'] . "' and status='1' and is_canceled='0' and is_deleted='0'  and invoice_date like '%" . $returnmonth . "%'";
                                                $b2bTotData = $obj_gstr1->get_results($b2bTotquery);
                                                $total = 0;
                                                if (!empty($b2bItemData)) {
                                                    $total = $b2bItemData[0]->cgst_amount + $b2bItemData[0]->sgst_amount + $b2bItemData[0]->igst_amount + $b2bItemData[0]->cess_amount;
...
$(document).ready(function () {
    $('#returnmonth').on('change', function () {
        document.form2.action = '<?php echo PROJECT_URL; ?>/?page=return_gstr1a_filling_summary&returnmonth=<?php echo $returnmonth; ?>';
