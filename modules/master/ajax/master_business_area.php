...
$aColumns = array('business_area_id', 'business_area_name', 'status', 'added_by', 'status');
$aSearchColumns = array('business_area_id', 'business_area_name');
$sIndexColumn = "business_area_id";

/* DB table to use */
//$sTable = $obj_master->getTableName('vendor_type');
$sTable = $obj_master->getTableName('business_area');
...
if (isset($_POST['iSortCol_0'])) {
    ...
    if ($sOrder == "ORDER BY ") {
        $sOrder = "ORDER BY business_area_id ASC";
    }
}
...
foreach($rResult as $aRow) {
    ...
    $row[] = $temp_x;
    $row[] = utf8_decode($aRow->business_area_name);
   	
    $row[] = $status;
    $row[] = '<a href="'.PROJECT_URL.'/?page=master_business_area_update&id='.$aRow->business_area_id.'" class="iconedit hint--bottom" data-hint="Edit" ><i class="fa fa-pencil"></i></a>';
    $output['aaData'][] = $row;
    $temp_x++;
}
}
