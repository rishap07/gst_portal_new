...
$aColumns = array('vendor_id', 'vendor_name', 'status', 'added_by', 'status');
$aSearchColumns = array('vendor_id', 'vendor_name');
$sIndexColumn = "vendor_id";
...
foreach($rResult as $aRow) {
    ...
    $row[] = $temp_x;
    $row[] = utf8_decode($aRow->vendor_name);
   	
    $row[] = $status;
    $row[] = '<a href="'.PROJECT_URL.'/?page=master_vendor_update&id='.$aRow->vendor_id.'" class="iconedit hint--bottom" data-hint="Edit" ><i class="fa fa-pencil"></i></a>';
    $output['aaData'][] = $row;
    $temp_x++;
}
}
