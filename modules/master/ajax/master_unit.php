 ...
 *  Last Modification   :   Unit Listing
 * 
*/
...
$aColumns = array('unit_id','unit_name','unit_code', 'status');
$aSearchColumns = array('unit_name','unit_code', 'status');
$sIndexColumn = "unit_id";

/* DB table to use */
$sTable = $obj_master->getTableName('unit');
...
if(isset($rResult) && !empty($rResult)){
	...
	foreach($rResult as $aRow) {
		...
		$row[] = $temp_x;
		$row[] = utf8_decode($aRow->unit_name);
		$row[] = utf8_decode($aRow->unit_code);
		$row[] = $status;
		$row[] = '<a href="'.PROJECT_URL.'/?page=master_unit_update&id='.$aRow->unit_id.'" class="iconedit hint--bottom" data-hint="Edit" ><i class="fa fa-pencil"></i></a>';
		$output['aaData'][] = $row;
		$temp_x++;
	}
}
