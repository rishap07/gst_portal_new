...
$aColumns = array('h.id', 'h.help_name','DATE_FORMAT(h.added_date, "%e %M %Y %r") as added_date', 'h.help_message');
$aSearchColumns = array('h.id', 'h.help_name','h.added_date');
$sIndexColumn = "h.id";
...
foreach($rResult as $aRow) {
    ...
    $row[] = $temp_x;
    $row[] = utf8_decode(ucfirst($aRow->help_name));
    $message=$obj_help->strip_tags_content(html_entity_decode(ucfirst($aRow->help_message)));
	$readmore='&nbsp<a href="'.PROJECT_URL.'/?page=help_view&id='.$aRow->id.'" class="iconedit hint--bottom" data-hint="Edit" >Read more</a>';
