<?php
	...
	if(!$obj_client->can_create('client_invoice')) {
		...
		$obj_client->redirect(PROJECT_URL."/?page=client_transportation_of_goods_invoice_list");
		exit();
	}

    $dataCurrentUserArr = $obj_client->getUserDetailsById( $obj_client->sanitize($_SESSION['user_detail']['user_id']) );
    $togInvoiceNumber = $obj_client->generateBillInvoiceNumber( $obj_client->sanitize($_SESSION['user_detail']['user_id']) );
?>
<!--POPUP START HERE-->
                ...
                <div class="kycform">
                    <div class="kycmainbox invoiceform">
						...
						<div class="formcol">
                            ...
                            <input type="text" placeholder="Invoice Serial Number" readonly="true" class="readonly required" value="<?php echo $togInvoiceNumber; ?>" name="invoice_serial_number" id="invoice_serial_number" />
                        </div>
						...
						<div class="formcol">
                            ...
                            <input type="text" placeholder="Number of Delivery Challan" class="required" data-bind="content" name="delivery_challan_number" id="delivery_challan_number">
                        </div>

						<div class="formcol two">
                            <label>Date of Delivery Challan<span class="starred">*</span></label>
                            <input type="text" placeholder="Date of Delivery Challan" class="required" data-bind="content" name="delivery_challan_date" id="delivery_challan_date">
                        </div>
		...
		$("#save_add_new_invoice").click(function(){
			...
			if(flag === 1) {
				...
				$.ajax({
					...
					dataType: 'json',
					type: 'post',
					url: "<?php echo PROJECT_URL; ?>/?ajax=client_save_transportation_of_goods_invoice",
					success: function(response){
						...
						if(response.status == "error") {
							...
							window.location.href = '<?php echo PROJECT_URL; ?>/?page=client_create_transportation_of_goods_invoice';
						}
					}
				});
			}
			...
			$.ajax({
                ...
                success: function(response){
                    ...
                    if(response.status == "error") {
                        ...
                        $(".errorValidationContainer").html("");
                        $(".errorValidationContainer").hide();
                        window.location.href = '<?php echo PROJECT_URL; ?>/?page=client_transportation_of_goods_invoice_list';
                    }
                }
            });
        });
