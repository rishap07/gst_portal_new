...
$invoiceErrorMessage = '';
$counter = 0;
if(isset($_POST['invoiceData']) && isset($_POST['action']) && $_POST['action'] == "saveNewPVInvoice" && isset($_GET['ajax']) && $_GET['ajax'] == "client_save_payment_voucher_invoice") {
	...
	if($obj_client->getErrorMessage() != '') {
		...
		if( !empty($invoiceItemArray) && count($invoiceItemArray) > 0 ) {
			...
			$dataArr['serial_number'] = $obj_client->generatePVInvoiceNumber( $obj_client->sanitize($_SESSION['user_detail']['user_id']) );

			if ($obj_client->insert($obj_client->getTableName('client_pv_invoice'), $dataArr)) {

				$insertid = $obj_client->getInsertID();
				$obj_client->logMsg("New Payment Voucher Invoice Added. ID : " . $insertid . ".");
				...
				if ($obj_client->insertMultiple($obj_client->getTableName('client_pv_invoice_item'), $processedInvoiceItemArray)) {

					$obj_client->setSuccess($obj_client->getValMsg('invoiceadded'));
					$iteminsertid = $obj_client->getInsertID();
					$obj_client->logMsg("New Payment Voucher Invoice Item Added. ID : " . $iteminsertid . ".");
