...
if(isset($_GET['invoiceDate']) && strtoupper($_GET['invoiceDate']) != "ALL") {
	$uWhere = " where ci.invoice_type = 'receiptvoucherinvoice' AND ci.is_deleted='0' AND ci.added_by='".$obj_client->sanitize($_SESSION['user_detail']['user_id'])."' AND DATE_FORMAT(invoice_date, '%Y-%m') = '".$_GET['invoiceDate']."' ";
} else {
	$uWhere = " where ci.invoice_type = 'receiptvoucherinvoice' AND ci.is_deleted='0' AND ci.added_by='".$obj_client->sanitize($_SESSION['user_detail']['user_id'])."' ";
}
...
$uQuery = "SELECT COUNT(" . $sIndexColumn . ") as count FROM $ciTable where invoice_type = 'receiptvoucherinvoice' AND is_deleted='0' AND added_by='".$obj_client->sanitize($_SESSION['user_detail']['user_id'])."'";
//echo $sQuery;
...
if(isset($rResult) && !empty($rResult)) {
    ...
    foreach($rResult as $aRow) {
		...
		$row[] = '<td><div class="list-primary pull-left"><div class="name"><a href="'.PROJECT_URL.'/?page=client_receipt_voucher_invoice_list&action=viewRVInvoice&id='.$aRow->invoice_id.'">'.$aRow->billing_name.'</a></div><a href="'.PROJECT_URL.'/?page=client_receipt_voucher_invoice_list&action=viewRVInvoice&id='.$aRow->invoice_id.'">'.$aRow->reference_number.'</a> | ' . $aRow->invoice_date . '</div><span class="pull-right"><div class="amount"><i class="fa fa-inr" aria-hidden="true"></i>'.$aRow->invoice_total_value.'</div><div class="greylinktext"><a href="'.PROJECT_URL.'/?page=client_update_receipt_voucher_invoice&action=editRVInvoice&id='.$aRow->invoice_id.'">Edit</a>&nbsp;&nbsp;'.$cancelLink.'</div></span></td></tr>';
