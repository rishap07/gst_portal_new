...
if(isset($_POST['correspondingType']) && isset($_POST['action']) && $_POST['action'] == "getCorrespondingType") {
	...
	if(count($correspondingTypeData) > 0) {
		...
		foreach($correspondingTypeData as $correspondingType) {
			$correspondingTypeItem .= '<option value="'. $correspondingType->invoice_id .'" data-serial="'. $correspondingType->serial_number .'" data-date="'. $correspondingType->invoice_date .'">'. $correspondingType->reference_number .'</option>';
		}
