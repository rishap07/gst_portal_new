...
$invoiceErrorMessage = '';
$counter = 0;
if(isset($_POST['invoiceData']) && isset($_POST['action']) && $_POST['action'] == "saveNewSTInvoice" && isset($_GET['ajax']) && $_GET['ajax'] == "client_save_special_tax_invoice") {
	...
	if($obj_client->getErrorMessage() != '') {
		...
		if( !empty($invoiceItemArray) && count($invoiceItemArray) > 0 ) {
			...
			$dataArr['serial_number'] = $obj_client->generateSTInvoiceNumber( $obj_client->sanitize($_SESSION['user_detail']['user_id']) );

			if ($obj_client->insert($obj_client->getTableName('client_st_invoice'), $dataArr)) {

				$insertid = $obj_client->getInsertID();
				$obj_client->logMsg("New Special Tax Invoice Added. ID : " . $insertid . ".");
				...
				if ($obj_client->insertMultiple($obj_client->getTableName('client_st_invoice_item'), $processedInvoiceItemArray)) {
					
					$obj_client->setSuccess($obj_client->getValMsg('invoiceadded'));
					$iteminsertid = $obj_client->getInsertID();
					$obj_client->logMsg("New Special Tax Invoice Item Added. ID : " . $iteminsertid . ".");
