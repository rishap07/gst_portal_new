...
if(isset($_POST['invoiceData']) && isset($_POST['action']) && $_POST['action'] == "saveNewBillInvoice" && isset($_GET['ajax']) && $_GET['ajax'] == "client_save_bill_of_supply_invoice") {
	...
	if($obj_client->getErrorMessage() != '') {
		...
		if( !empty($invoiceItemArray) && count($invoiceItemArray) > 0 ) {
			...
			if ($obj_client->insert($obj_client->getTableName('client_bos_invoice'), $dataArr)) {
				...
				if ($obj_client->insertMultiple($obj_client->getTableName('client_bos_invoice_item'), $processedInvoiceItemArray)) {
