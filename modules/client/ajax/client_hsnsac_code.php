...
if(isset($_GET['ajax']) && $_GET['ajax'] == "client_hsnsac_code" && isset($_GET['term'])) {
	...
	$clientItem = $obj_client->get_results("select i.item_id, CONCAT(UCASE(LEFT(i.item_name,1)),LCASE(SUBSTRING(i.item_name,2))) as item_name, i.hsn_code, i.item_type, i.igst_tax_rate, i.csgt_tax_rate, i.sgst_tax_rate, i.cess_tax_rate from " . $obj_client->getTableName('item') . " as i where 1=1 AND i.status = '1' AND i.is_deleted='0' AND (i.item_name LIKE '%".trim($_GET['term'])."%' OR i.hsn_code LIKE '%".trim($_GET['term'])."%') ORDER BY i.item_name ASC");
