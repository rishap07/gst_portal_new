...
if(isset($_GET['ajax']) && $_GET['ajax'] == "client_hsnsac_code" && isset($_GET['term'])) {
	...
	$clientItem = $obj_client->get_results("select i.item_id, i.item_name, i.hsn_code, i.item_type from " . $obj_client->getTableName('item') . " as i where 1=1 AND i.status = '1' AND i.is_deleted='0' AND (i.item_name LIKE '%".trim($_GET['term'])."%' OR i.hsn_code LIKE '%".trim($_GET['term'])."%') ORDER BY i.item_name ASC");
