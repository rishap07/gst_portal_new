...
$counter = 0;
$errorcounter = 1;
if(isset($_POST['invoiceData']) && isset($_POST['action']) && $_POST['action'] == "saveUpdateBillInvoice" && isset($_GET['ajax']) && $_GET['ajax'] == "client_save_update_bill_of_supply_invoice") {
	...
	if($invoiceErrorMessageContent != '') {
		...
		if( !empty($invoiceItemArray) && count($invoiceItemArray) > 0 ) {
			...
			if ($obj_client->update($obj_client->getTableName('client_invoice'), $dataArr, $dataConditionArray)) {
				...
				$obj_client->logMsg("Bill of Supply Invoice Updated. ID : " . $updatedid . ".");
