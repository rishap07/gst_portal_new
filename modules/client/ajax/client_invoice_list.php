...
$aColumns = array('ci.invoice_id', 'ci.invoice_type', 'ci.invoice_nature', 'ci.serial_number', 'ci.reference_number', 'ci.company_name', 'ci.company_address', 'ci.gstin_number', 'ci.supply_type', 'ci.invoice_date', 'ci.is_canceled', 'ci.invoice_total_value', 'ci.billing_name', 'ci.shipping_name');

$aSearchColumns = array('ci.invoice_type', 'ci.invoice_nature', 'ci.serial_number', 'ci.reference_number', 'ci.invoice_date', 'ci.invoice_total_value', 'ci.billing_name', 'ci.shipping_name');
...
if(isset($_GET['invoiceDate']) && strtoupper($_GET['invoiceDate']) != "ALL") {
	$uWhere = " where ci.invoice_type IN('taxinvoice','exportinvoice','sezunitinvoice','deemedexportinvoice') AND ci.is_deleted='0' AND ci.added_by='".$obj_client->sanitize($_SESSION['user_detail']['user_id'])."' AND DATE_FORMAT(invoice_date, '%Y-%m') = '".$_GET['invoiceDate']."' ";
} else {
	$uWhere = " where ci.invoice_type IN('taxinvoice','exportinvoice','sezunitinvoice','deemedexportinvoice') AND ci.is_deleted='0' AND ci.added_by='".$obj_client->sanitize($_SESSION['user_detail']['user_id'])."' ";
}
...
$uQuery = "SELECT COUNT(" . $sIndexColumn . ") as count FROM $ciTable where invoice_type IN('taxinvoice','exportinvoice','sezunitinvoice','deemedexportinvoice') AND is_deleted='0' AND added_by='".$obj_client->sanitize($_SESSION['user_detail']['user_id'])."'";
//echo $sQuery;
...
if(isset($rResult) && !empty($rResult)) {
    ...
    foreach($rResult as $aRow) {
		...
		if($aRow->invoice_type == 'taxinvoice') {
			$row[] = '<td><div class="list-primary pull-left"><div class="name"><a href="'.PROJECT_URL.'/?page=client_invoice_list&action=viewInvoice&id='.$aRow->invoice_id.'">'.$aRow->billing_name.'</a></div><a href="'.PROJECT_URL.'/?page=client_invoice_list&action=viewInvoice&id='.$aRow->invoice_id.'">'.$aRow->reference_number.'</a> | ' . $aRow->invoice_date . '</div><span class="pull-right"><div class="amount"><i class="fa fa-inr" aria-hidden="true"></i>'.$aRow->invoice_total_value.'</div><div class="greylinktext"><a href="'.PROJECT_URL.'/?page=client_update_invoice&action=editInvoice&id='.$aRow->invoice_id.'">Edit</a>&nbsp;&nbsp;'.$cancelLink.'</div></span></td></tr>';
		} else {
