<?php
	...
	if (!$obj_client->can_read('client_kyc_by_subscriber')) {
		$obj_client->setError($obj_client->getValMsg('can_read'));
		$obj_client->redirect(PROJECT_URL . "/?page=dashboard");
		exit();
	}

	if (!$obj_client->can_create('client_kyc_by_subscriber')) {
		$obj_client->setError($obj_client->getValMsg('can_create'));
		$obj_client->redirect(PROJECT_URL . "/?page=dashboard");
		exit();
	}

	if (!$obj_client->can_update('client_kyc_by_subscriber')) {
		$obj_client->setError($obj_client->getValMsg('can_update'));
		$obj_client->redirect(PROJECT_URL . "/?page=dashboard");
		exit();
	}
	
	if( isset($_GET['id']) && $obj_client->validateId($_GET['id']) && isset($_GET['action']) && $_GET['action'] == "updateClientKYC") {

		$clientID = $obj_client->sanitize($_GET['id']);
		$cuData = $obj_client->get_row("select user_id from " . $obj_client->getTableName('user') ." where user_id = '".$clientID."' AND added_by = '".$obj_client->sanitize($_SESSION['user_detail']['user_id'])."' AND is_deleted='0'");
		if (empty($cuData)) {
			$obj_client->setError("No client found.");
			$obj_client->redirect(PROJECT_URL."?page=dashboard");
		}
	} else {
		$obj_client->setError("Invalid client.");
		$obj_client->redirect(PROJECT_URL . "/?page=dashboard");
		exit();
	}

	$dataArr = array();
	$dataArr = $obj_client->getUserDetailsById($clientID);
	$redirectURL = PROJECT_URL . "?page=client_kycupdate_by_subscriber&action=updateClientKYC&id=" . $clientID;
	if (isset($_POST['submit']) && $_POST['submit'] == 'submit') {
		...
		if (!isset($_SERVER['HTTP_REFERER']) || empty($_SERVER['HTTP_REFERER'])) {
			...
			if ($obj_client->saveClientKYCBySubscriber()) {
				$obj_client->redirect($redirectURL);
			}
		}
	}
?>
...
<script>
	$(document).ready(function () {
		...
		/* submit kyc form */
		$('.formboxcontainer').on('submit', 'form#client-kyc', function(event){
            ...
            $.ajax({
				...
				dataType: 'json',
                type: 'post',
                url: "<?php echo PROJECT_URL; ?>/?ajax=client_submit_kyc_by_subscriber",
