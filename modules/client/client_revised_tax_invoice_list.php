            ...
            <div class="row">
                ...
                <div class="fixed-left-col col-sm-12 col-xs-12" style="padding-right:0px; padding-left:0px;">
                    ...
                    <div class="invoiceheaderfixed">
						...
						<?php $invoiceMonthYear = $obj_client->getInvoiceMonthList($obj_client->getTableName('client_invoice'), "'revisedtaxinvoice','creditnote','debitnote'"); ?>
                ...
                if (isset($_GET['action']) && $_GET['action'] == 'viewRTInvoice' && isset($_GET['id']) && $obj_client->validateId($_GET['id'])) {
					...
					$invoiceData = $obj_client->get_results("select 
												...
												cii.total 
												from 
											" . $obj_client->getTableName('client_invoice') . " as ci INNER JOIN " . $obj_client->getTableName('client_invoice_item') . " as cii ON ci.invoice_id = cii.invoice_id where ci.invoice_id = (SELECT invoice_id FROM ".$obj_client->getTableName('client_invoice')." Where invoice_type IN('revisedtaxinvoice', 'creditnote', 'debitnote') AND added_by = '" . $obj_client->sanitize($_SESSION['user_detail']['user_id']) . "' AND is_deleted='0' Order by invoice_id desc limit 0,1) AND ci.invoice_type IN('revisedtaxinvoice', 'creditnote', 'debitnote') AND ci.added_by = '" . $obj_client->sanitize($_SESSION['user_detail']['user_id']) . "' AND cii.added_by = '" . $obj_client->sanitize($_SESSION['user_detail']['user_id']) . "' AND ci.is_deleted='0' AND cii.is_deleted='0'");
				}
        ...
        return {
            init: function () {
                ...
                $('#mainTable').dataTable({
                    ...
                    "searching": false,
                    "bLengthChange": false,
                    "sAjaxSource": "<?php echo PROJECT_URL; ?>/?ajax=client_revised_tax_invoice_list&invoiceDate=" + $("#invoiceDateOption option:selected").val(),
