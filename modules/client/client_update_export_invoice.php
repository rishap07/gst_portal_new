<?php
	...
	if( isset($_GET['action']) && $_GET['action'] == 'editInvoice' && isset($_GET['id']) && $obj_client->validateId($_GET['id'])) {
		...
		$invoiceData = $obj_client->get_results("select 
													...
													cii.total 
													from 
												" . $obj_client->getTableName('client_invoice') . " as ci INNER JOIN " . $obj_client->getTableName('client_invoice_item') . " as cii ON ci.invoice_id = cii.invoice_id where ci.invoice_id = " . $invid . " AND ci.invoice_type IN('exportinvoice','sezunitinvoice','deemedexportinvoice') AND ci.added_by = '" . $obj_client->sanitize($_SESSION['user_detail']['user_id']) . "' AND cii.added_by = '" . $obj_client->sanitize($_SESSION['user_detail']['user_id']) . "' AND ci.is_deleted='0' AND cii.is_deleted='0'");
...
<form name="create-invoice" id="create-invoice" method="POST">
	...
	<?php if($invoiceData[0]->export_supply_meant === "withoutpayment") { ?>
			...
			<div class="whitebg formboxcontainer">
				...
				<div class="row">
					<div class="col-md-5 col-sm-5 col-xs-12 form-group">
						...
						<label class="radio-inline"><input type="radio" name="invoice_type" value="exportinvoice" <?php if($invoiceData[0]->invoice_type === "exportinvoice") { echo 'checked="checked"'; } ?>>Export</label>
							...
							<?php if(!empty($dataReceiptVoucherArrs)) { ?>
								...
								<?php foreach($dataReceiptVoucherArrs as $dataReceiptVoucherArr) { ?>
									...
									<?php if($invoiceData[0]->receipt_voucher_number === $dataReceiptVoucherArr->invoice_id) { ?>
										<option value='<?php echo $dataReceiptVoucherArr->invoice_id; ?>' data-date="<?php echo $dataReceiptVoucherArr->invoice_date; ?>" selected="selected"><?php echo $dataReceiptVoucherArr->reference_number; ?></option>
									<?php } else { ?>
										<option value='<?php echo $dataReceiptVoucherArr->invoice_id; ?>' data-date="<?php echo $dataReceiptVoucherArr->invoice_date; ?>"><?php echo $dataReceiptVoucherArr->reference_number; ?></option>
									<?php } ?>
				 ...
				 <div class="row">
					...
					<div class="col-md-6">
						<div class="greyborder inovicedeatil">
							...
							<div class="row form-group">
								<div class="col-md-4 col-sm-3 col-xs-12 padleftnone"><label>GSTIN/UIN</label></div>
								<div class="col-md-8 col-sm-3 col-xs-12"><input type="text" placeholder="GSTIN/UIN" class="form-control" name='shipping_gstin_number' data-bind="gstin" id='shipping_gstin_number' value="<?php echo $invoiceData[0]->shipping_gstin_number; ?>" /></div>
							</div>
							...
							<div class="row form-group exportinformation">
								<div class="col-md-4 col-sm-3 col-xs-12 padleftnone"><label>Export Bill Port Code</label></div>
								<div class="col-md-8 col-sm-3 col-xs-12">
									<input type="text" placeholder="Export Bill Port Code" name='export_bill_port_code' class="form-control" id='export_bill_port_code' data-bind="content" value="<?php echo $invoiceData[0]->export_bill_port_code; ?>" />
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="4" class="table invoicetable tablecontent">
						...
						<?php foreach($invoiceData as $invData) { ?>
							...
							<tr class="invoice_tr" data-row-id="<?php echo $counter; ?>" id="invoice_tr_<?php echo $counter; ?>">
								...
								<?php if($invoiceData[0]->export_supply_meant == "withpayment") { ?>
