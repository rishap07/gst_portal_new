...
if(isset($rResult) && !empty($rResult)) {
    ...
    foreach($rResult as $aRow) {
		...
		$row[] = '<td><div class="list-primary pull-left"><div class="name"><a href="'.PROJECT_URL.'/?page=purchase_revised_tax_invoice_list&action=viewPurchaseRTInvoice&id='.$aRow->purchase_invoice_id.'">'.$aRow->supplier_billing_name.'</a></div><a href="'.PROJECT_URL.'/?page=purchase_revised_tax_invoice_list&action=viewPurchaseRTInvoice&id='.$aRow->purchase_invoice_id.'">'.$aRow->serial_number.'</a> | ' . $aRow->invoice_date . '</div><span class="pull-right"><div class="amount"><i class="fa fa-inr" aria-hidden="true"></i>'.$aRow->invoice_total_value.'</div><div class="greylinktext"><a href="'.PROJECT_URL.'/?page=purchase_revised_tax_invoice_update&action=editPurchaseRTInvoice&id='.$aRow->purchase_invoice_id.'">Edit</a>&nbsp;&nbsp;'.$cancelLink.'</div></span></td></tr>';
