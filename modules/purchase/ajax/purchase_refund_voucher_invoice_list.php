...
if(isset($_GET['invoiceDate']) && strtoupper($_GET['invoiceDate']) != "ALL") {	
	$uWhere = " where ci.invoice_type = 'refundvoucherinvoice' AND ci.is_deleted='0' AND ci.added_by='".$obj_purchase->sanitize($_SESSION['user_detail']['user_id'])."' AND DATE_FORMAT(invoice_date, '%Y-%m') = '".$_GET['invoiceDate']."' ";
} else {
	$uWhere = " where ci.invoice_type = 'refundvoucherinvoice' AND ci.is_deleted='0' AND ci.added_by='".$obj_purchase->sanitize($_SESSION['user_detail']['user_id'])."' ";
}
...
$uQuery = "SELECT COUNT(" . $sIndexColumn . ") as count FROM $ciTable where invoice_type = 'refundvoucherinvoice' AND is_deleted='0' AND added_by='".$obj_purchase->sanitize($_SESSION['user_detail']['user_id'])."'";
//echo $sQuery;
...
if(isset($rResult) && !empty($rResult)) {
    ...
    foreach($rResult as $aRow) {
		...
		$row[] = '<td><div class="list-primary pull-left"><div class="name"><a href="'.PROJECT_URL.'/?page=purchase_refund_voucher_invoice_list&action=viewPurchaseRFInvoice&id='.$aRow->purchase_invoice_id.'">'.$aRow->supplier_billing_name.'</a></div><a href="'.PROJECT_URL.'/?page=purchase_refund_voucher_invoice_list&action=viewPurchaseRFInvoice&id='.$aRow->purchase_invoice_id.'">'.$aRow->reference_number.'</a> | ' . $aRow->invoice_date . '</div><span class="pull-right"><div class="amount"><i class="fa fa-inr" aria-hidden="true"></i>'.$aRow->invoice_total_value.'</div><div class="greylinktext"><a href="'.PROJECT_URL.'/?page=purchase_refund_voucher_invoice_update&action=editPurchaseRFInvoice&id='.$aRow->purchase_invoice_id.'">Edit</a>&nbsp;&nbsp;'.$cancelLink.'</div></span></td></tr>';
