...
$counter = 0;
$errorcounter = 1;
if(isset($_POST['invoiceData']) && isset($_POST['action']) && $_POST['action'] == "saveNewPurchaseRTInvoice" && isset($_GET['ajax']) && $_GET['ajax'] == "purchase_revised_tax_invoice_save") {
	...
	if($invoiceErrorMessageContent != '') {
		...
		if( !empty($invoiceItemArray) && count($invoiceItemArray) > 0 ) {
			...
			$dataArr['serial_number'] = $obj_purchase->generatePurchaseRTInvoiceNumber( $obj_purchase->sanitize($_SESSION['user_detail']['user_id']) );

			if ($obj_purchase->insert($obj_purchase->getTableName('client_purchase_invoice'), $dataArr)) {

				$insertid = $obj_purchase->getInsertID();
				$obj_purchase->logMsg("Purchase Revised Tax Invoice Added. ID : " . $insertid . ".");
				...
				if ($obj_purchase->insertMultiple($obj_purchase->getTableName('client_purchase_invoice_item'), $processedInvoiceItemArray)) {
					...
					$obj_purchase->logMsg("Purchase Revised Tax Invoice Item Added. ID : " . $iteminsertid . ".");
