...
$counter = 0;
$errorcounter = 1;
if(isset($_POST['invoiceData']) && isset($_POST['action']) && $_POST['action'] == "saveUpdatePurchaseRFInvoice" && isset($_GET['ajax']) && $_GET['ajax'] == "purchase_refund_voucher_invoice_save_update") {
	...
	if($invoiceErrorMessageContent != '') {
		...
		if( !empty($invoiceItemArray) && count($invoiceItemArray) > 0 ) {
			...
			if ($obj_purchase->update($obj_purchase->getTableName('client_purchase_invoice'), $dataArr, $dataConditionArray)) {
				...
				$obj_purchase->logMsg("Purchase Refund Voucher Invoice Updated. ID : " . $updatedid . ".");
