...
$supplier = array();
$counter = 0;
if(isset($_GET['ajax']) && $_GET['ajax'] == "purchase_get_supplier_by_business_name" && isset($_GET['term'])) {
	
	$clientGetSuppliers = $obj_purchase->get_results("select su.supplier_id, su.name, su.company_name, su.email, su.address, su.city, su.zipcode, su.phone, su.fax, su.pannumber, su.gstid, su.website, su.remarks, su.vendor_type, s.state_id, s.state_name, s.state_code, s.state_tin, c.id as country_id, c.country_code, c.country_name from " . $obj_purchase->getTableName('supplier') . " as su left join " . $obj_purchase->getTableName('state') . " as s on su.state = s.state_id left join " . $obj_purchase->getTableName('country') . " as c on su.country = c.id where 1=1 AND su.is_deleted='0' AND su.status = '1' AND su.company_name LIKE '%".trim($_GET['term'])."%' AND su.added_by = '".$obj_purchase->sanitize($_SESSION['user_detail']['user_id'])."'");
	...
	if(count($clientGetSuppliers) > 0) {
		...
		foreach($clientGetSuppliers as $clientGetSupplier) {
			...
			$supplier[$counter]['name'] = html_entity_decode($clientGetSupplier->name);
			$supplier[$counter]['label'] = html_entity_decode($clientGetSupplier->company_name);
			$supplier[$counter]['value'] = html_entity_decode($clientGetSupplier->company_name);
