...
if(isset($_GET['ajax']) && $_GET['ajax'] == "purchase_get_supplier_by_business_name" && isset($_GET['term'])) {
	...
	$clientGetSuppliers = $obj_purchase->get_results("select su.supplier_id, su.name, su.company_name, su.email, su.address, su.city, su.zipcode, su.phone, su.fax, su.pannumber, su.gstid, su.website, su.remarks, s.state_id, s.state_name, s.state_code, s.state_tin from " . $obj_purchase->getTableName('supplier') . " as su, " . $obj_purchase->getTableName('state') . " as s where 1=1 AND su.state = s.state_id AND su.is_deleted='0' AND su.status = '1' AND su.company_name LIKE '%".trim($_GET['term'])."%' AND su.added_by = '".$obj_purchase->sanitize($_SESSION['user_detail']['user_id'])."'");
	...
	if(count($clientGetSuppliers) > 0) {
		...
		foreach($clientGetSuppliers as $clientGetSupplier) {
			...
			$supplier[$counter]['name'] = $clientGetSupplier->name;
			$supplier[$counter]['label'] = $clientGetSupplier->company_name;
			$supplier[$counter]['value'] = $clientGetSupplier->company_name;
